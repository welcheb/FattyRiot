%% Function name: FattyRiot_fw_i2cm1i_3pluspoint_hernando
%%
...
%%   - algoParams.fm: initial guess for fieldmap (Hz) 
%%
...
%% Date last modified: March 1, 2013 (welcheb)

function outParams = FattyRiot_fw_i2cm1i_3pluspoint_hernando_graphcut( imDataParams, algoParams )

DEBUG = 0;

% Check validity of params, and set default algorithm parameters if not provided
[validParams,algoParams] = FattyRiot_checkParamsAndSetDefaults_graphcut( imDataParams,algoParams );
if validParams==0
...
try 
  ...
  if UNIFORM_TEs == 1 % TEST DH* 090801
    ...
    residual = FattyRiot_computeResidual( imDataParams, algoParams );
...
dfm = fms(2)-fms(1);
lmap = FattyRiot_getQuadraticApprox( residual, dfm );  
...
try
    fm = algoParams.fm;
    disp('FattyRiot: FattyRiot_fw_i2cm1i_3pluspoint_hernando.m : Using provided fieldmap');
catch
    % This is the core of the algorithm
    fm = FattyRiot_graphCutIterations(imDataParams,algoParams,residual,lmap,cur_ind );
end

% force call to graphCutIterations
% disp('fw_i2cm1i_3pluspoint_hernando.m : force call to graphCutIterations');
...
for ka=1:size(imDataParams.images,6) 
  ...
  if algoParams.range_r2star(2)>0
    ...
    r2starmap(:,:,ka) = FattyRiot_estimateR2starGivenFieldmap( curParams, algoParams, fm );
  else
  ...
  if DO_OT ~= 1
    ...
    amps = FattyRiot_decomposeGivenFieldMapAndDampings( curParams, algoParams, fm, r2starmap(:,:,ka), r2starmap(:,:,ka) );
...
if DO_OT == 1
  ...
  outParams = FattyRiot_fw_i2cm0i_3plusploint_hernando_optimtransfer( imDataParams, algoParams  );      
