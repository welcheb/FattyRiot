...
[masksignal,resLocalMinima,numMinimaPerVoxel] = FattyRiot_findLocalMinima( residual, 0.06 );
...
try
    fm = algoParams.fm;
    disp('graphCutIterations.m : Using provided fieldmap');
catch
    fm = zeros(sx,sy);
end
...
% $$$   mymax = max(cur_ind(:))
  ...
  if DISPLAY_ITER == 1
    ...
    imagesc(fm);axis off equal tight,colormap gray;colorbar;
